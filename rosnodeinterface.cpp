...
// Generated 30-Sep-2022 17:11:04
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"explicitaccel\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(explicitaccel_M, (NULL));
        explicitaccel_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(explicitaccel_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(explicitaccel_M)
                == (NULL)) || rtmGetStopRequested(explicitaccel_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(explicitaccel_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
